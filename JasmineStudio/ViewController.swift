...
//  OutlineViewDemo
//
//  Created by Gabriel Theodoropoulos. Folded, spindled, and mutitlated by Steve Gravrock.
...
//    
    ...
    @MainActor @objc func shenanigans(outlineView: NSOutlineView) {
...
//         Do any additional setup after loading the view.
        ...
        models = [MyModel("a"), MyModel("b"), MyModel("c")]
        inited = true
        print("reloading")
...
extension ViewController: NSOutlineViewDataSource {
    func outlineView(_ outlineView: NSOutlineView, numberOfChildrenOfItem item: Any?) -> Int {
        if (self.models == nil) {
            print("numberOfChildrenOfItem bailing early");
            return 0
        } else if (item == nil) {
            let n = self.models!.count
            print("numberOfChildrenOfItem: root, \(n))")
            return n
        } else {
            print("numberOfChildrenOfItem: non root, 0")
            return 0
        }
//        let ret = item == nil ? viewModel.model.totalCollections : (item as? Collection)?.totalItems ?? 1
//        print("outlineView:numberOfChildrenOfItem:\(String(describing: item)) == \(ret)")
//        return ret
    }
    
    
    func outlineView(_ outlineView: NSOutlineView, child index: Int, ofItem item: Any?) -> Any {
        print("child ofItem: \(item)")
        return models![index]
...
//        } else {
            ...
            guard let cell = outlineView.makeView(withIdentifier: cellIdentifier, owner: nil) as? NSTableCellView else { return nil }
        cell.textField?.stringValue = "O HAI"
