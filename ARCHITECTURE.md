...
Jasmine Studio displays the suites and specs in an NSOutlineView at all times
after the initial enumeration completes. That view is is backed by a tree of
suite and spec model objects. In order to update the outline view, the following
jobs need to be done in response to reporter events generated by a test run:

* Update the state of existing tree nodes (status, failed expectations, debug
  logs, etc.)
* Remove nodes corresponding to specs/suites that no longer exist
* Add nodes for newly discovered suites/specs 


The user can change the set of suites and specs by editing the source code 
while Jasmine Studio is open. They can also rename specs and suites. Since the
full path (i.e. the names of the node and its ancestors) is the only stable
identifier for a node, a rename is indistinguishable from an add and a remove.

A straightforwrd, naive approach is to emit a new tree and then rebind the
NSOutlineView to it. That won't work for two reasons. First, users need to be
able to see the result of individual specs without waiting for the entire test
run to continue. Second, NSOutlineView [identifies the models backing
individual rows/tree nodes by their memory addresses](https://developer.apple.com/documentation/appkit/nsoutlineviewdatasource?language=objc).
Replacing the tree is possible, but it means losing UI state such as expanded
subtrees, the current selection, etc.

To avoid that, Jasmine Studio needs to update the existing tree to match the new
structure and state revealed during each execution. That's done with something
like a pipeline.  As individual reporter events are emitted:

1. JasmineRunner (low-level wrapper around the `jasmine` CLI) decodes the events
   and emits delegate calls.
2. The decoded events are passed to ReporterTreeBuilder. It creates new nodes
   from suite/spec started events and updates those nodes with data from suite/
	spec done events. It then passes the resulting nodes to a delegate method.
3. The emitted nodes are passed to TreeReconciler. TreeReconciler has a
   reference to the shared tree structure that backs the NSOutlineView. If the
	emitted node represents an already known suite/spec, TreeReconciler updates
	the node's state and emits a delegate call indicating that the node has
	changed. Otherwise, it inserts a new node and emits a delegate call
	indicating that the new node's parent has changed.
4. The NSOutlineView controller is the TreeReconciler's delegate. It responds
   to delegate method calls by telling the NSOutlineView to reload affected
	nodes/subtrees.

To handle deletion, TreeReconciler tracks which nodes have been touched during
the current test run. At the end (i.e. in response to the jasmineDone reporter
event) it removes untouched nodes and emits delegate calls indicating that each
node's parent has changed. This is done from the root up so that only a single
delegate call is made when an entire subtree is removed. One drawback to
detecting deletion in jasmineDone is that some removed nodes will still be
displayed after their removal is knowable. That could be fixed by implementing
deletion detection for individual subtrees in suiteDone.
